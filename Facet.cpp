...
Facet::Facet(int nbIndex) {
  ...
  //visible[5]=1; //Troll statement to corrupt heap
}
...
DWORD Facet::GetHitsSize(int nbMoments) {
  ...
  return   sizeof(SHHITS)
        ...
        + (sh.isProfile?(PROFILE_SIZE*sizeof(llong)*(1+nbMoments)):0)
...
float Facet::GetSmooth(int i,int j,AHIT *texBuffer,float scaleF) {
...
void Facet::BuildTexture(AHIT *texBuffer,AHIT min,AHIT max,BOOL useColorMap,float dCoeff,BOOL doLog,AHIT correctionFactor) {
  ...
  if( useColorMap ) {
    ...
    for(int j=0;j<sh.texHeight;j++) {
      for(int i=0;i<sh.texWidth;i++) {
        ...
        if( doLog ) {
          val = (int)((LOG10(texBuffer[idx]*dCoeff*correctionFactor)-LOG10(min))*scaleFactor+0.5f);
        } else {
          val = (int)((texBuffer[idx]*dCoeff*correctionFactor-min)*scaleFactor+0.5f);
        }
    ...
    if( mesh ) {
      for(int j=-1;j<=sh.texHeight;j++) {
        for(int i=-1;i<=sh.texWidth;i++) {
          ...
          if( doSmooth ) {
            if( doLog ) {
              val = (int)((LOG10(GetSmooth(i,j,texBuffer,dCoeff*correctionFactor))-LOG10(min))*scaleFactor+0.5f);
            } else {
              val = (int)((GetSmooth(i,j,texBuffer,dCoeff*correctionFactor)-min)*scaleFactor+0.5f);
            }
            SATURATE(val,0,65535);
            buff32[(i+1) + (j+1)*texDimW] = colorMap[val];
          }
