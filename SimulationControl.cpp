...
BOOL LoadSimulation(Dataport *loader) {
  ...
  for(i=0;i<sHandle->totalFacet;i++) {
    ...
    if(f->sh.isTextured) {
      ...
      f->inc = (AHIT *)malloc(f->textureSize);
    ...
    if(f->sh.isProfile) {
      f->profileSize = PROFILE_SIZE*sizeof(llong);
      /*f->profile = (llong *)malloc(f->profileSize);
      memset(f->profile,0,f->profileSize);*/
		if ((f->profile=(llong **)malloc(sizeof(llong *) * (1+sHandle->nbMoments)))==NULL) {
			ReleaseDataport(loader);
			SetErrorSub("Couldn't allocate memory (time moments container, profiles)");
			return FALSE;
		}
		memset(f->profile,0,sizeof(llong *) * (1+sHandle->nbMoments)); //set all pointers to NULL so we can use SAFE_FREE later
		for (int m=0;m<(1+sHandle->nbMoments);m++) {
			if ((f->profile[m] = (llong *)malloc(f->profileSize))==NULL) { //steady-state plus one for each moment
				ReleaseDataport(loader);
...
//printf("Resetcounter called.");
  sHandle->tmpCount.hit.nbHit = 0;
  sHandle->tmpCount.hit.nbDesorbed = 0;
  sHandle->tmpCount.hit.nbAbsorbed = 0;
