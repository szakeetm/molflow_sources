              ...
              for(y=0;y<f->sh.texHeight;y++) {           
                ...
                if( f->fullElem[add]) {
                  ...
                  if( RaySphereIntersect(&center,r,rPos,rDir,&d) ) {
                    if( d<intMinLgth ) {
						f->direction[0][add].dir.x += sHandle->pDir.x*sHandle->velocityCurrentParticle;
						f->direction[0][add].dir.y += sHandle->pDir.y*sHandle->velocityCurrentParticle;
						f->direction[0][add].dir.z += sHandle->pDir.z*sHandle->velocityCurrentParticle;
						f->direction[0][add].sumSpeed += sHandle->velocityCurrentParticle;
						...
						for (size_t m=0;m<nbMoments;m++) {
							if (time<sHandle->moments[m]) {
								f->direction[m+1][add].dir.x += sHandle->pDir.x*sHandle->velocityCurrentParticle;
								f->direction[m+1][add].dir.y += sHandle->pDir.y*sHandle->velocityCurrentParticle;
								f->direction[m+1][add].dir.z += sHandle->pDir.z*sHandle->velocityCurrentParticle;
								f->direction[m+1][add].sumSpeed += sHandle->velocityCurrentParticle;
								f->direction[m+1][add].count++;
							}
						}
					}
				  }
