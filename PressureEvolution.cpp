	...
	for(int i=0;i<nbView;i++) {
		...
		if( v->userData>=0 && v->userData<geom->GetNbFacet()) {
			...
			if (modeCombo->GetSelectedIndex()==1) //plot one slice
				val=(double)profilePtr[selectedSlice];
			else { //plot sum/average
				llong valLLong=0;
				for (int j=0;j<PROFILE_SIZE;j++)
					valLLong+=profilePtr[j];
				val=(double)valLLong;
			...
			case 6:
				...
				break;
			case 7: //Pressure
				scale = 40.0*totalOutgassing*(worker->desorptionStopTime-worker->desorptionStartTime)
