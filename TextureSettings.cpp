...
void TextureSettings::Update() {
  ...
  char tmp[128];
  sprintf(tmp,"%.3E",(geom->texAutoScaleIncludeConstantFlow?geom->texMinAutoscale:geom->texMinAutoscaleMomentsOnly));
  texCMinText->SetText(tmp);
  sprintf(tmp,"%.3E",(geom->texAutoScaleIncludeConstantFlow?geom->texMaxAutoscale:geom->texMaxAutoscaleMomentsOnly));
  ...
  if( !geom->texAutoScale ) {
    gradient->SetMinMax((double)geom->texMin,(double)geom->texMax);
  } else {
    gradient->SetMinMax(
		(double)(geom->texAutoScaleIncludeConstantFlow?geom->texMinAutoscale:geom->texMinAutoscaleMomentsOnly),
		(double)(geom->texAutoScaleIncludeConstantFlow?geom->texMaxAutoscale:geom->texMaxAutoscaleMomentsOnly)
		);
  }
...
void TextureSettings::Display(Worker *w,GeometryViewer **v) {
  ...
  char tmp[128];
  sprintf(tmp,"%.3E",geom->texMin);
  texMinText->SetText(tmp);
  sprintf(tmp,"%.3E",geom->texMax);
  ...
  switch(message) {
    ...
    if (src==updateButton) {
      ...
      Update();
		...
		geom->texMin=(geom->texAutoScaleIncludeConstantFlow?geom->texMinAutoscale:geom->texMinAutoscaleMomentsOnly);
		geom->texMax=(geom->texAutoScaleIncludeConstantFlow?geom->texMaxAutoscale:geom->texMaxAutoscaleMomentsOnly);
