...
size_t SubprocessFacet::InitializeTexture(const size_t &nbMoments)
{
    ...
    if (sh.isTextured) {
        ...
        double fullSizeInc = (sh.texWidthD * sh.texHeightD) / (geo.U.Norme() * geo.V.Norme());
...
void SubprocessFacet::InitializeOutgassingMap()
{
    if (sh.useOutgassingFile) {
        ...
        ogMap.outgassingMapWidthD = geo.U.Norme() * ogMap.outgassingFileRatio;
        ogMap.outgassingMapHeightD = geo.V.Norme() * ogMap.outgassingFileRatio;
...
size_t SubprocessFacet::GetMemSize() const {
    ...
    sum += sizeof (Vector2d) * geo.vertices2.capacity();
...
int SimulationModel::InitialiseFacets() {
    for (auto& facet : facets) {
        ...
        facet.geo.CalculateFacetParams(vertices3, facet.indices);
