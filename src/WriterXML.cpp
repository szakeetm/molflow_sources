...
int WriterXML::SaveSimulationState(std::string outputFileName, SimulationModel *model, BYTE* buffer) {
    xml_document saveDoc;
    ...
    for (size_t m = 0; m <= model->tdParams.moments.size(); m++) {
        ...
        if (m == 0) { //Write global results. Later these results will probably be time-dependent as well.
            ...
            for (size_t i = 0; i < gHits->hitCacheSize; i++) {
                ...
                newHit.append_attribute("posY") = gHits->hitCache[i].pos.y;
                newHit.append_attribute("posZ") = gHits->hitCache[i].pos.z;
                newHit.append_attribute("type") = gHits->hitCache[i].type;
            }

            xml_node leakCacheNode = globalNode.append_child("Leak_Cache");
            leakCacheNode.append_attribute("nb") = gHits->leakCacheSize;
            for (int i = 0; i < gHits->leakCacheSize; i++) {
                xml_node newLeak = leakCacheNode.append_child("Leak");
                newLeak.append_attribute("id") = i;
                newLeak.append_attribute("posX") = gHits->leakCache[i].pos.x;
                newLeak.append_attribute("posY") = gHits->leakCache[i].pos.y;
                newLeak.append_attribute("posZ") = gHits->leakCache[i].pos.z;
                newLeak.append_attribute("dirX") = gHits->leakCache[i].dir.x;
                newLeak.append_attribute("dirY") = gHits->leakCache[i].dir.y;
                newLeak.append_attribute("dirZ") = gHits->leakCache[i].dir.z;
            }
        } //end global node
