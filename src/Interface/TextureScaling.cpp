...
TextureScaling::TextureScaling(Worker* worker_, GeometryViewer** viewers_):GLWindow(),
    ...
    autoscaleTimedepModeCombo->SetValueAt(1,"Moments and constant flow");
...
void TextureScaling::ProcessMessage(GLComponent *src,int message) {
	...
	case MSG_BUTTON:
		...
		if (src==updateButton) {
			...
			manualScaleMinText->SetText(geomMinLabel->GetText());
			manualScaleMaxText->SetText(geomMaxLabel->GetText());
