...
int main(int argc, char** argv) {
	...
	if (!SettingsIO::autogenerateTest) {
		try {
			Initializer::initFromFile(&simManager, model, &simuState, persistentUserSettings);
#if defined(USE_MPI)
			MPI_Finalize();
#endif
		}
    ...
    else if(SettingsIO::autogenerateTest && Initializer::initAutoGenerated(&simManager, model, &simuState,
                                                                           10.0, 10, M_PI_4)){
#if defined(USE_MPI)
        MPI_Finalize();
#endif
    ...
    if (Settings::simDuration > 0) {
        Log::console_msg_master(1, "[{}] Preparing simulation for {} seconds from {} desorptions...\n", Util::getTimepointString(), Settings::simDuration, simuState.globalStats.globalHits.nbDesorbed);
    }
