...
int MolFlow::FrameMove()
{
	...
	if ((elapsedTime <= 2.0f) && runningState) {
		...
		sprintf(tmp, "%s (%s)", Util::formatInt(worker.globalState.globalStats.globalHits.nbMCHit, "hit"),
		...
		sprintf(tmp, "%s (%s)", Util::formatInt(worker.globalState.globalStats.globalHits.nbDesorbed, "des"),
...
void MolFlow::LoadFile(const std::string& fileName) {
	...
	try {
		...
		ClearFacetParams();
		nbDesStart = worker.globalState.globalStats.globalHits.nbDesorbed;
		nbHitStart = worker.globalState.globalStats.globalHits.nbMCHit;
		AddRecent(filePath);
		geom->viewStruct = -1;
...
void MolFlow::StartStopSimulation() {
	...
	if (worker.globalState.globalStats.globalHits.nbMCHit <= 0 && !worker.model->wp.calcConstantFlow && worker.moments.empty()) {
	...
	lastNbHit = worker.globalState.globalStats.globalHits.nbMCHit;
	lastNbDes = worker.globalState.globalStats.globalHits.nbDesorbed;
