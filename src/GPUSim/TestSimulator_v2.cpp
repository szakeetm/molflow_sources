...
int mainV2(int argc, char** argv) {
    ...
    if(!SettingsIO::autogenerateTest && InitializerGPU::initFromFile(&simManager, model, &globState)){
#if defined(USE_MPI)
        MPI_Finalize();
#endif
        return 42;
    }
    else if(SettingsIO::autogenerateTest && InitializerGPU::initAutoGenerated(&simManager, model, &globState,
                                                                           10.0, 10, M_PI_4)){
#if defined(USE_MPI)
        MPI_Finalize();
#endif
