...
*/
        ...
        if(angle > 0.0 && Initializer::initAutoGenerated(&simManager, simModel, &globState, 100.0, 10, DegToRad(angle))){
#if defined(USE_MPI)
            MPI_Finalize();
#endif
            return 43;
        }
        if(!fileName.empty() && Initializer::initFromFile(&simManager, simModel, &globState)){
#if defined(USE_MPI)
            MPI_Finalize();
#endif
            return 42;
