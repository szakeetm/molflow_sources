...
#include "GLWindow.h"
#include "GLTextArea.h"
#include "GLToolkit.h"

// ------------------------------------------------------

GLTextArea::GLTextArea(int compId,char *text):GLComponent(compId) {
...
char *GLTextArea::GetText() {
  return m_Text;
}

// ------------------------------------------------------

void GLTextArea::Clear() {
  SetText("");
}

// ------------------------------------------------------

void GLTextArea::SetCursorPos(int pos) {
...
int GLTextArea::GetCursorPos() {
  return m_CursorPos;
}

// ------------------------------------------------------

int GLTextArea::GetTextLength() {
  return m_Length;
}

// ------------------------------------------------------

void GLTextArea::SetEditable(BOOL editable) {
  m_Editable = editable;
  if( m_Editable ) {
	  SetCursor(CURSOR_TEXT);
	  SetBackgroundColor(240,240,240);
...
void GLTextArea::SetEditable_NoBG(BOOL editable) { //for combo boxes
  m_Editable = editable;
  if( m_Editable ) {
	  SetCursor(CURSOR_TEXT);
  }
...
BOOL GLTextArea::IsCaptured() {
  return m_Captured;
}

// ------------------------------------------------------

void GLTextArea::SetText(char *text) {
...
void GLTextArea::UpdateText(char *text) {
  if(text) {
    strncpy(m_Text,text,MAX_TEXTAREA_SIZE);
    m_Text[MAX_TEXTAREA_SIZE-1]=0;
  } else
    strcpy(m_Text,"");
  UpdateXpos();
}
...
BOOL GLTextArea::GetNumber(double *num) {
...
// ------------------------------------------------------
BOOL GLTextArea::GetNumberInt(int *num) {
...
void GLTextArea::ScrollToVisible()
{
...
void GLTextArea::InsertString(char *lpszString)
{
  char tmp[MAX_TEXTAREA_SIZE];
  if(!m_Editable) return;

  DeleteSel();

  if( (strlen(lpszString) + strlen(m_Text)) < MAX_TEXTAREA_SIZE ) {
    strcpy(tmp,m_Text);
    tmp[m_CursorPos]=0;
...
void GLTextArea::DeleteString(int count)
{
  if(!m_Editable) return;
...
void GLTextArea::MoveCursor(int newPos)
{
  if( m_CursorState ) {
    m_CursorState=0;
  }
...
void GLTextArea::MoveSel(int newPos)
{
  m_Stop=newPos;
  m_CursorPos=newPos;
  ScrollToVisible();
...
void  GLTextArea::CopyClipboardText() {
...
void GLTextArea::PasteClipboardText() {
...
void GLTextArea::SetFocus(BOOL focus) {
...
int GLTextArea::GetCursorLocation(int px) {
...
void GLTextArea::ManageEvent(SDL_Event *evt) {
